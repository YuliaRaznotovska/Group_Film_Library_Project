...
def init_user_lists(user_id):
    default_lists = ["Watched", "Favourite", "Recommend", "Don't recommend"]
    existing_lists = database.db_session.execute(select(models.List).where(models.List.user_id == user_id)).scalars().all()
    existing_names = [film_list.name for film_list in existing_lists]
    for list_name in default_lists:
        if list_name not in existing_names:
            new_list = models.List(name=list_name, user_id=user_id)
...
def get_user_id(user_id):
    ...
    if user_by_session is not None:
        ...
        user_lists = database.db_session.execute(
            select(models.List).where(models.List.user_id == session.get('user_id'))).scalars().all()
    return render_template('user_page.html', user=user_by_id, user_session=user_by_session, user_lists=user_lists)
...
def get_delete_user_form(user_id):
    database.init_db()
    user_by_session = session.get('user_id')
    user_by_id = database.db_session.execute(select(models.User).where(models.User.id == user_id)).scalar_one_or_none()
    if user_by_session is not None:
        user_by_session = database.db_session.execute(
            select(models.User).where(models.User.id == user_by_session)).scalar_one_or_none()
    return render_template('delete_user_account.html', user=user_by_id, user_session=user_by_session)
...
def delete_user_id(user_id):
    ...
    if session_user_id == int(user_id):
        database.db_session.execute(delete(models.User).where(models.User.id == session_user_id))
        session.clear()
        database.db_session.commit()
        return redirect(url_for('main_page'))
    else:
...
def get_film(film_id):
    ...
    feedbacks= database.db_session.execute(select(models.Feedback, models.User.first_name, models.User.last_name).join(models.User, models.User.id == models.Feedback.user_id).where(models.Feedback.film_id == film_id)).all()
    return render_template('film_profile.html', film_list=result, actors=actors, genres=genres, feedbacks=feedbacks)


@app.route('/films/update/<film_id>', methods=['GET'])
@decorator_check_login
def update_film(film_id):
    ...
    genres = database.db_session.execute(
        select(models.Genre)
        .join(models.GenreFilm, models.Genre.genre == models.GenreFilm.genre_id)
        .where(models.GenreFilm.film_id == film_id)
    ).scalars().all()
    return render_template('film_update.html', film_list=result, actors=actors, genres=genres)

@app.route('/films/update/<film_id>', methods=['POST'])
@decorator_check_login
def update_film_submit(film_id):
    film = database.db_session.execute(select(models.Film).where(models.Film.id == film_id)).scalar_one()
    film.name = request.form.get('name')
    film.year = request.form.get('year')
    film.description = request.form.get('description')
    film.duration = request.form.get('duration')
    film.country = request.form.get('country')
    film.poster = request.form.get('poster')
    film.added_info = request.form.get('added_info')
    actor_first_name_input = request.form["actor_first_name"]
    actor_last_name_input = request.form["actor_last_name"]
    actors_first_names = [actor.strip() for actor in actor_first_name_input.split(",")
                          if actor_first_name_input.strip()]
    ...
    database.db_session.execute(
        delete(models.ActorFilm).where(models.ActorFilm.film_id == film.id)
    )
    for first_name, last_name in zip(actors_first_names, actors_last_names):
        actor_obj = database.db_session.execute(select(models.Actor).where(
            models.Actor.actor_first_name == first_name,
    ...
    genre_names = [genre.strip() for genre in genre_input.split(",") if genre.strip()]
    database.db_session.execute(
        delete(models.GenreFilm).where(models.GenreFilm.film_id == film.id)
    )
    for name in genre_names:
        genre_obj = database.db_session.execute(
            select(models.Genre).where(models.Genre.genre == name)).scalar_one_or_none()
...
def get_rating(film_id):
    films = database.db_session.execute(select(models.Film).where(models.Film.id == film_id)).scalars().all()
    return render_template("rating_update.html", film_list=films)
...
def post_rating(film_id):
    ...
    if film.rating is None:
        ...
        film.rating = (film.rating + new_rating) / 2
        database.db_session.commit()
    return redirect(f'/films/{film.id}')


@app.route('/films/<film_id>/feedback/add', methods=['GET'])
def get_feedback_form(film_id):
    film = database.db_session.execute(select(models.Film).where(models.Film.id == film_id)).scalar_one()
    return render_template('add_comment.html', film=film)


@app.route('/films/<film_id>/feedback/add', methods=['POST'])
def add_feedback(film_id):
    database.init_db()
    comment = request.form["feedback"]
    grade = database.db_session.execute(select(models.Film.rating).where(models.Film.id == film_id)).scalar_one()
    feedback = database.db_session.execute(select(models.Feedback).where(models.Feedback.user_id == session["user_id"]).where(models.Feedback.film_id == film_id)).scalar_one_or_none()
    if feedback:
        feedback.grade = grade
        feedback.description = comment
    else:
        feedback = models.Feedback(film_id=int(film_id),user_id=session['user_id'], grade=grade, description=comment)
        database.db_session.add(feedback)
    database.db_session.commit()
...
@app.route('/films/<int:film_id>/add', methods=['POST'])
@decorator_check_login
def add_film_to_watch_list(film_id):
    database.init_db()
    user_id = session.get('user_id')
    list_name = request.form['list_name']

    user_list = database.db_session.execute(select(models.List).where(models.List.user_id == user_id, models.List.name == list_name)).scalar_one_or_none()

    existing = database.db_session.execute(select(models.FilmList).where(models.FilmList.film_id == film_id, models.FilmList.list_id == user_list.id)).scalar_one_or_none()

    if not existing:
        add_to_list = models.FilmList(film_id=film_id, list_id=user_list.id)
        database.db_session.add(add_to_list)
        database.db_session.commit()
...
@decorator_check_login
def delete_film_from_watch_list(user_id, list_id):
    film_id = request.form.get('film_id')

    film_in_list = database.db_session.execute(select(models.FilmList).where(models.FilmList.list_id == list_id, models.FilmList.film_id == film_id)).scalar_one_or_none()

    if film_in_list:
        database.db_session.delete(film_in_list)
        database.db_session.commit()
...
def get_user_list(user_id, list_id):
    database.init_db()
    films = database.db_session.execute(select(models.Film).join(models.FilmList).where(models.FilmList.list_id == list_id)).scalars().all()
    user_list = database.db_session.execute(select(models.List).where(models.List.id == list_id)).scalar_one_or_none()
    return render_template('user_list.html', films=films, list_name=user_list.name, user_list=user_list)
